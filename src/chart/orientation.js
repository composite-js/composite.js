import { CHART_TYPE_REGISTRY, getChartTypeDefinition } from "./registry.js";

export const ORIENTATION_INFERRED_CHART_TYPES = new Set(
  Object.entries(CHART_TYPE_REGISTRY)
    .filter(([, definition]) => definition.inferredOrientation)
    .map(([mark]) => mark),
);

export function isOrientationInferredChartType(mark) {
  return getChartTypeDefinition(mark)?.inferredOrientation === true;
}
