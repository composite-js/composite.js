...
export class ExportSvgElement {
  ...
  toString() {
    ...
    const text = this._textContent ? escapeText(this._textContent) : "";
    return `<${this.tagName}${attributes}>${text}${children}</${this.tagName}>`;
  }
}

export class ExportSvgDocument {
  constructor() {
    this.documentElement = new ExportSvgElement("html", this, XHTML_NS);
    this.body = new ExportSvgElement("body", this, XHTML_NS);
    this.head = new ExportSvgElement("head", this, XHTML_NS);
    this.documentElement.appendChild(this.head);
    this.documentElement.appendChild(this.body);
  }

  createElement(tagName) {
    return new ExportSvgElement(tagName, this, XHTML_NS);
  }

  createElementNS(namespaceURI, tagName) {
    return new ExportSvgElement(tagName, this, namespaceURI);
  }
}

export async function withExportSvgDocument(callback) {
  const previousDocument = globalThis.document;
  const previousSVGElement = globalThis.SVGElement;
  const previousWindow = globalThis.window;
  const document = new ExportSvgDocument();

  globalThis.document = document;
  globalThis.SVGElement = ExportSvgElement;
